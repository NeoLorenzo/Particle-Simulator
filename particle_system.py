...
def _resolve_collision_jit(i, j, positions, velocities, masses, radii, temperatures, heat_coeff, restitution, g_const, softening):
    ...
    if distance_sq < min_distance**2 and distance_sq > 0:
        ...
        temp_i_before, temp_j_before = temperatures[i][0], temperatures[j][0]
        ...
        # 3. Calculate Net Energy Change and Redistribute as Heat
        ...
        # Crucially, a collision cannot create negative heat (make things colder).
        # This prevents an unphysical scenario where resolving a deep overlap
        # with low kinetic energy loss would cool the system.
        heat_from_impact = max(0.0, ke_lost - pe_change)

        # Conductive heat transfer: energy flows from hot to cold
        # Q = k * (T_hot - T_cold)
        # This is a simplified model. The amount transferred is proportional
        # to the difference and the heat transfer coefficient.
        temp_diff = temp_i_before - temp_j_before
        heat_transferred = heat_coeff * temp_diff # heat from i to j

        # Apply all thermal changes.
        # The total thermal energy added to the pair is from the impact.
        # This energy is then distributed, and on top of that, heat is
        # exchanged between the two particles.
        
        # Update particle i's temperature
        # It gets half the impact heat, and loses heat to particle j
        delta_temp_i = (0.5 * heat_from_impact - heat_transferred) / m_i
        temperatures[i][0] += delta_temp_i

        # Update particle j's temperature
        # It gets half the impact heat, and gains heat from particle i
        delta_temp_j = (0.5 * heat_from_impact + heat_transferred) / m_j
        temperatures[j][0] += delta_temp_j
